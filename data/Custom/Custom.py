...
class Custom(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.root_dir = osp.join('..', 'data', 'Custom', 'data')
    ...
    def load_data(self):
        db = COCO(osp.join(self.root_dir, "custom_{}_data.json".format(self.data_split)))
        ...
        for aid in db.anns.keys():
            ...
            if self.data_split == 'train':
                ...
                xs, ys = [], []
                hand_bbox = ann['hand_bbox']
                object_bbox = ann['object_bbox']

                if hand_bbox is not None:
                    xs.extend([hand_bbox[0], hand_bbox[0] + hand_bbox[2]])
                    ys.extend([hand_bbox[1], hand_bbox[1] + hand_bbox[3]])
                if object_bbox is not None:
                    xs.extend([object_bbox[0], object_bbox[0] + object_bbox[2]])
                    ys.extend([object_bbox[1], object_bbox[1] + object_bbox[3]])
                if len(xs) == 0:
                    continue
                # xs = [hand_bbox[0], hand_bbox[0] + hand_bbox[2], object_bbox[0], object_bbox[0] + object_bbox[2]]
                # ys = [hand_bbox[1], hand_bbox[1] + hand_bbox[3], object_bbox[1], object_bbox[1] + object_bbox[3]]
                ...
                mano_pose = np.array(ann['mano_param']['mano_pose'], dtype=np.float32)[0]
                mano_shape = np.array(ann['mano_param']['mano_shape'], dtype=np.float32)[0]
    ...
    def __getitem__(self, idx):
        ...
        expand_ratio = np.random.uniform(1.3-0.2, 1.3+0.2)
        bbox = process_bbox(bbox, img_shape[1], img_shape[0], expansion_factor=expand_ratio)
